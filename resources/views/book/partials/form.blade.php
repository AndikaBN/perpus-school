<form action="{{ route('buku.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-4">
        <x-input-label for="ebook">{{ __('E-book') }}</x-input-label>
        <x-text-input id="ebook" class="mt-1 block w-full" type="file" name="ebook" />
        @error('ebook')
            <x-input-error-set :message="$message" class="mt-2" />
        @enderror
    </div>
    ...
    <div class="mb-4">
        <x-input-label for="cover">{{ __('Cover') }}</x-input-label>
        <x-text-input id="cover" class="mt-1 block w-full" type="file" name="cover" />
        @error('cover')
            <x-input-error-set :message="$message" class="mt-2" />
        @enderror
    </div>
